...
class CarTracking(threading.Thread):
    def __init__(self):
        ...
        self.state_gains = [2.0,2/3*360/(np.pi*2),2.0,360/(np.pi*2), 2.0, 2*360/(np.pi*2)]
        self.dtarget = 100
        self.yawtarget = 0.0
...
#         # self.vxyz = np.array([vyw,vzw,vxw])
        ...
        while self.local_target_number < len(self.local_targets_z):
            ...
            if self.local_target_number < 118 :
                ...
                274-2*recover_dist<self.local_target_number<314+recover_dist or \
                    424-recover_dist<self.local_target_number<464+2*recover_dist or 1124-4*recover_dist<self.local_target_number<1164+2*recover_dist or 1264-2*recover_dist<self.local_target_number<1304+2*recover_dist or 1654-30<self.local_target_number<1694 or \
                        699-2*recover_dist<self.local_target_number<739+2*recover_dist or 1504-4*recover_dist<self.local_target_number<1544+70:
    ...
    #     self.local_targets_z33 = np.array(np.linspace(-118,119,240))
    #     self.local_targets_x33 = np.array(np.repeat(0.5,240))
    #     angle_arc_34 = np.linspace(np.pi, np.pi/2, 40)
    #     for i in range(len(angle_arc_34)-1):
    #         self.local_targets_z34.append(119.5 + 7.5 * np.sin(angle_arc_34[i]))
    #         self.local_targets_x34.append(8 + 7.5 * np.cos(angle_arc_34[i]))
    ...
    #     #Segment 2
    #     self.local_targets_z_s2 = np.concatenate(((self.local_targets_z5, self.local_targets_z6, self.local_targets_z7, self.local_targets_z8, self.local_targets_z9, self.local_targets_z10, self.local_targets_z11)))
    #     self.local_targets_x_s2 = np.concatenate(((self.local_targets_x5, self.local_targets_x6, self.local_targets_x7, self.local_targets_x8, self.local_targets_x9, self.local_targets_x10, self.local_targets_x11)))
    ...
    #     # time.sleep(self.arrowspeed * (len(self.local_targets_z_s2) - len(self.local_targets_z5)))
    #     # self.client4.simFlushPersistentMarkers()    
    #     while self.local_target_number < 900: #988:
    #         time.sleep(0.1)
    #     self.client4.simFlushPersistentMarkers()       
        
    #     #Segment 3
    #     self.local_targets_z_s3 = np.concatenate(((self.local_targets_z11, self.local_targets_z12, self.local_targets_z13, self.local_targets_z14, self.local_targets_z15, self.local_targets_z16, self.local_targets_z17)))
    #     self.local_targets_x_s3 = np.concatenate(((self.local_targets_x11, self.local_targets_x12, self.local_targets_x13, self.local_targets_x14, self.local_targets_x15, self.local_targets_x16, self.local_targets_x17)))
    ...
    #     # self.client4.simFlushPersistentMarkers()
    #     while self.local_target_number < 1370: #1448:
    #         time.sleep(0.1)
    #     self.client4.simFlushPersistentMarkers()   

    #     #Segment 4
    #     self.local_targets_z_s4 = np.concatenate(((self.local_targets_z17, self.local_targets_z18, self.local_targets_z19, self.local_targets_z20, self.local_targets_z21)))
    #     self.local_targets_x_s4 = np.concatenate(((self.local_targets_x17, self.local_targets_x18, self.local_targets_x19, self.local_targets_x20, self.local_targets_x21)))
    ...
    #     # self.client4.simFlushPersistentMarkers()
    #     while self.local_target_number < 1970: #2003:
    #         time.sleep(0.1)
    #     self.client4.simFlushPersistentMarkers()   

    #     #Segment 5
    #     self.local_targets_z_s5 = np.concatenate(((self.local_targets_z21_, self.local_targets_z22, self.local_targets_z23, self.local_targets_z24, self.local_targets_z25, self.local_targets_z26, self.local_targets_z27, self.local_targets_z28, self.local_targets_z29)))
    #     self.local_targets_x_s5 = np.concatenate(((self.local_targets_x21_, self.local_targets_x22, self.local_targets_x23, self.local_targets_x24, self.local_targets_x25, self.local_targets_x26, self.local_targets_x27, self.local_targets_x28, self.local_targets_x29)))
    ...
    #     # self.client4.simFlushPersistentMarkers()
    #     while self.local_target_number < 2760: #2817:
    #         time.sleep(0.1)
    #     self.client4.simFlushPersistentMarkers()

    #     #Segment 6
    #     self.local_targets_z_s6 = np.concatenate(((self.local_targets_z29, self.local_targets_z30, self.local_targets_z31, self.local_targets_z32, self.local_targets_z33, self.local_targets_z34, self.local_targets_z35)))
    #     self.local_targets_x_s6 = np.concatenate(((self.local_targets_x29, self.local_targets_x30, self.local_targets_x31, self.local_targets_x32, self.local_targets_x33, self.local_targets_x34, self.local_targets_x35)))
    ...
    #             self.arrowspeed = 0.2
    #             time.sleep(self.arrowspeed)
    #         elif 119<self.local_target_number<159+recover_dist or 848-recover_dist<self.local_target_number<888+recover_dist or 2003-recover_dist<self.local_target_number<2043+recover_dist or \
    #             273-recover_dist<self.local_target_number<313+recover_dist or 988-recover_dist<self.local_target_number<1028+recover_dist or 1723-recover_dist<self.local_target_number<1763+recover_dist or 2687-recover_dist<self.local_target_number<2727+recover_dist or \
    #                 423-recover_dist<self.local_target_number<463+2*recover_dist or 1058-recover_dist<self.local_target_number<1098+recover_dist or 1448-recover_dist<self.local_target_number<1488+recover_dist or 2407-recover_dist<self.local_target_number<2447+recover_dist or 2817-recover_dist<self.local_target_number<2857+recover_dist or \
    #                     698-2*recover_dist<self.local_target_number<738+recover_dist or 1208-recover_dist<self.local_target_number<1248+recover_dist or 2243-recover_dist<self.local_target_number<2283+recover_dist or 3057-recover_dist<self.local_target_number<3097+recover_dist:
    #             self.arrowspeed = 0.25
    #             time.sleep(self.arrowspeed)
    #         else:
    #             self.arrowspeed = 0.1
    ...
    def carclustering(self,img):
        ...
        if np.abs(self.current_pos[4]) < 0.1:
            ...
            self.zthres_obs = 0.05 * self.current_pos[4]**2 + 0.5 * self.current_pos[4] + 16
...
#         self.dyaw = 2
    ...
    def run(self):    # Experiment B (with Car2)
        ...
        while True:
            ...
            if (110 - position_car1.x_val) < 32 and self.preprocessing.local_target_number > 1500 and flag == 0:
                self.car_control()
                time.sleep(0.1)
                t1.start()
                t1thread_start = 1
